...
fromAll : {ℓ : Level} {p : (z : ℕ) → Set ℓ} {a a1 a2 : ℕ}
          {a3 : B23Tree a a1 a2} (b : B23TreeAll p a3) →
          B23TreeP p a a1 a2
fromAll (node₀ ⦃ _ ⦄) = node₀
fromAll {a3 = (node₂ x t u)} (node₂ x₁ x₂ x₃) = node₂ x x₁
                                                (fromAll x₂) (fromAll x₃)
fromAll {a3 = (node₃ x y t u v)} (node₃ x₁ x₂ x₃ x₄ x₅) = node₃ x
                                                          x₁ y x₂ (fromAll x₃) (fromAll x₄)
                                                          (fromAll x₅)
...
toAll : {ℓ : Level} {p : (z : ℕ) → Set ℓ} {a a1 a2 : ℕ}
        (n : B23TreeP p a a1 a2) →
        B23TreeAll p (toB23Tree n)
toAll (node₀ ⦃ _ ⦄) = node₀
toAll (node₂ x x₁ x₂ x₃) = node₂ x₁ (toAll x₂) (toAll x₃)
toAll (node₃ x x₁ y x₂ x₃ x₄ x₅) = node₃ x₁ x₂ (toAll x₃)
                                   (toAll x₄) (toAll x₅)
