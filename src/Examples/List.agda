...
unquoteDecl data Len constructor c0′ c1′ = defineByDataD LenD Len (c0′ ∷ c1′ ∷ [])
-- data Len {A : Set ℓ} : ℕ → List A → Set ℓ where
--   zero : Len 0 []
--   suc  : {a : A} {n : ℕ} {as : List A} → Len n as → Len (suc n) (a ∷ as)

LenC = genDataC LenD (genDataT LenD Len)
...
unquoteDecl data ListP constructor c0′′ c1′′ = defineByDataD ListPD  ListP (c0′′ ∷ c1′′ ∷ [])
...
ListPC = genDataC ListPD (genDataT ListPD ListP)

ListPFin : Finitary ListPD
ListPFin = [] ∷ (tt ∷ tt ∷ refl ∷ []) ∷ []
...
-- instance
ListAllC = genDataC ListAllD ListAllT
  where ListAllT = genDataT ListAllD ListAll
...
ListAnyD = ⌊ ListAnyOD ⌋ᵈ

unquoteDecl data ListAny constructor cAny0 cAny1 = defineByDataD ⌊ ListAnyOD ⌋ᵈ ListAny (cAny0 ∷ cAny1 ∷ [])
...
ListAnyC = genDataC ListAnyD ListAnyT
  where ListAnyT = genDataT ListAnyD ListAny

_∋_ : {A : Set ℓ} → List A → A → Set ℓ
xs ∋ x = ListAny _ (x ≡_) xs
