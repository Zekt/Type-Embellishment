...
private
  ...
  conClause rec pars #levels Γps c = do
    ...
    --dprint [ strErr "clause generated:" ]
    --dprint [ strErr cl ]
  ...
  renameType : Type → List String → Type
  renameType (`Π[ _ ∶ a ] x) (s ∷ ss) = `Π[ s ∶ a ] (renameType x ss)
  renameType t _ = t
...
defineFold P f = do
  ...
  cls ← extendContextℓs #levels λ ℓs → do
    ...
    --dprint [ strErr $ "telescope:" ]
    --dprint [ strErr $ show Γps ]
