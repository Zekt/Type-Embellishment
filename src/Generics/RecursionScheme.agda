...
FoldOpTelᶜˢ []       X = []
FoldOpTelᶜˢ (D ∷ Ds) X = FoldOpTᶜ D X ∷ const (FoldOpTelᶜˢ Ds X)
...
Homᶜˢ (D ∷ Ds) {X} (f , fs) (g , gs) h =
  (∀ {i} (xs : ⟦ D ⟧ᶜ X i) → Homᶜ D f g h xs) ∷ const (Homᶜˢ Ds fs gs h)
...
IndOpTelᶜˢ []       f P = []
IndOpTelᶜˢ (D ∷ Ds) f P = IndOpTᶜ D (f ∘ inl) P ∷ const (IndOpTelᶜˢ Ds (f ∘ inr) P)
