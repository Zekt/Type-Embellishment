...
Algᶜ : {I : Set ℓⁱ} (D : ConD I ℓʳ ℓᵃ cρ) → (I → Set ℓ)
     → Set (ℓ ⊔ ℓⁱ ⊔ ℓʳ ⊔ ℓᵃ ⊔ lcond cρ ℓ)
Algᶜ D X = ∀ {i} → ⟦ D ⟧ᶜ X i → X i

Algᶜˢ : {I : Set ℓⁱ} (Ds : ConDs I ℓʳ ℓᵃ cρs) → (I → Set ℓ)
      → Set (ℓ ⊔ ℓⁱ ⊔ ℓʳ ⊔ ℓᵃ ⊔ lcond (length cρs) ℓⁱ ⊔ lconds cρs ℓ)
...
Algᵈ : (D : DataD) → Carrierᵈ D ℓ
     → Set (ℓ ⊔ DataD.plevel D ⊔ DataD.ilevel D ⊔ DataD.rlevel D ⊔ DataD.alevel D
