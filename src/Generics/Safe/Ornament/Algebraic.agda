...
algODᶜ : {I : Set ℓⁱ} (D : ConD I ℓᵃ cρ) {X : I → Set ℓ}
       → Algᶜ D X → ConOD (Σ I X) fst D (ℓᵃ ⊔ lcond cρ ℓ)
...
algODᶜˢ : {I : Set ℓⁱ} (D : ConDs I ℓᵃ cρs) {X : I → Set ℓ}
        → Algᶜˢ D X → ConODs (Σ I X) fst D (ℓᵃ ⊔ lconds cρs ℓ) cρs
algODᶜˢ ∅        alg = ∅
...
algODᵘᵖᵈ D {X} alg = record
  ...
  ; levels  = id
  ; OrnDesc = λ ℓs → algODᵈ (UPDataD.Desc D ℓs) alg }