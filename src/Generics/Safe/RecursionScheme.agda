...
open import Prelude
open import Generics.Safe.Telescope; open ∀ℓω; open ∀ᵗ
...
fold-algᵖᵈ : (D : PDataD) → ∀ {ℓ} → ∀ᵗ false _ λ ps → FoldAlgTᵖᵈ D ps ℓ
fold-algᵖᵈ D $$ ps $$ fs = algebra _ (fold-algᶜˢ (PDataD.applyP D ps) fs)

fold-algᵈ : (D : DataD) → ∀ℓω _ λ ℓs → ∀ {ℓ} → ∀ᵗ false _ λ ps → FoldAlgTᵈ D ℓs ps ℓ
