...
In this section, we will show how elaborator reflection is used to streamline generic programming:
First, macros are used to translate representations described in \Cref{sec:parameters} used by generic programs such as |Tel : Level → Set|, which are \emph{higher-order} and \emph{typed}, and those by the reflection framework which are \emph{first-order} and \emph{uni-typed}.
...
We should, however, sketch some notable features of the reflection framework (\Cref{sec:elab}) and then explain how that is used in subsequent sections (\Cref{sec:translation,sec:connection-generation,sec:specialising}).

\subsection{Elaborator Reflection in Agda}
\label{sec:elab}
Agda's reflection API includes a set of primitives based on the elaborator monad, called |TC|, which operates on the reflected language formed by |Tm| (\Cref{fig:reflected-term}), |Pattern| (\Cref{fig:reflected pattern}), |Literal| (\Cref{fig:other-type}), |Clause| (\Cref{fig:clause}), and |Definition| (\Cref{fig:definition}), corresponding to each syntactic category.
Every reflected expression is in weak head normal form and all application are in spine-normal form.
The reflected expression can be derived by the construct |quoteTerm| and the resolved unique name for a definition or a constructor by |quote|.  
For example, the reflected expression |quoteTerm acc| of the constructor |acc| is equal to |con (quote acc) []| where the empty list |[]| indicates that |acc| alone has no arguments.
Note that for illustrative purposes, a small subset of reflected expressions is presented instead of the actual type |Term| of reflected expression (\Cref{fig:full reflected syntax}) which carries the information of arguments' visibility and modality.
The impact of addressing the additional information will be discussed in \Cref{sec:examples}, but for now they are not important for our understanding.

The elaborator monad |TC| encloses states needed for elaboration, including the current context, the scope of names |f : Name| with its associated type and definition, the set of metavariables |x : Meta|, and so on.
...
\end{minipage}%
\end{figure}
A \emph{macro} is just a special TC computation of type |Term → TC ⊤| and a definition of type |A₁ → ... → Term → TC ⊤| declared as |macro| will be expanded during elaboration. 
The call site of a macro |m| is treated as a metavariable $x$ and the macro |m| is applied to the reflected expression |meta x []| upon expansion. 
Let the primitive |unify : Term → Term → TC ⊤| which unifies two given reflected expressions be declared as a macro called |give| (\Cref{fig:give}). 
Then, |give e| would splice the given reflected expression~|e| in place of the call if Agda did nothing to its argument |e|.
In fact, Agda quotes arguments of type |Term| and |Name| before macro expansions, so |give e| is invoked as |unify (quoteTerm e)|. 
Hence, the expression |give e| is just equal to |e|.

The reflected language in Agda is based on the de Bruijn representation and thus \emph{not} hygiene.
Manipulating indices directly is notoriously error-prone, but in many cases there is no need to do so.
...
Yet we can unquote the reflected expression instead and bind it to an (object-level) variable.
...
We will see some concrete examples in \Cref{sec:translation}. 

\subsection{Translation between Typed Higher-Order and Uni-typed First-Order Representations}
\label{sec:translation}
\LT{explain |quoteTC| and |unquoteTC| here}

Our generic programs are given in the typed and higher-order representations, so to use macros to generate native datatypes and definitions we have to first translate them to its corresponding first-order and uni-typed representation.
Take telescopes for example. 
A reflected telescopes is just a list |Γ : List Type| of reflected types, while an inhabitant |T : Tel ℓ| (\Cref{fig:telescopes}) used as part of the datatype representation |DataD| may consists of functions.
One obvious approach is to analyse the reflected expressions.
For instance, we would need to translate the quoted expression of |[ A ∶ Set ] [ R ∶ (A → A → Set) ] []|, i.e.\ 
\begin{code}
def (quote ∷-syntax) (`Set 0 ∷ lam
  (def (quote ∷-syntax) (pi (var 0 []) (pi (var 1 []) (`Set 0)) ∷ lam
    (con (quote Tel.[]) []) ∷ [])) ∷ [])
\end{code}
to |`Set 0 ∷ pi (var 0 []) (pi (var 1 []) (`Set 0)) ∷ []| where |`Set 0| reflects the type |Set|.
Naively a macro would need to pattern match against |t : Tm| and check which constructor is being analysed by reducing the reflected expression |def f xs| to one of |con (quote Tel.[]) xs|, |con (quote Tel._∷_) xs|, and |con (quote Tel._++_) xs|.
In other words, the elaborator would give up checking types for us and we essentially write a program for \emph{untyped} abstract syntax trees.

Instead of quoting the entire telescope |T| at once and pattern matching against the reflected expression, we choose to pattern match against |T : Tel ℓ| first. 
The cases for the empty telescope and for appending telescopes are rather straightforward to write
\begin{code}
...
...
\end{code}
The remaining problem is to retain a telescope given a function |T : A → Tel ℓ'| in which an argument of type |A| is needed.
\begin{code}
fromTel (A ∷ T)   = do
  extendContextT A λ `A x → do
    Γ ← fromTel (T x) (U x)
    return (`A ∷ Γ)
\end{code}
