...
For the translation from |DataD|, a |TC| computation |defineByDataD| is defined using the same technique of creating local variables, although the datatype signature needs some attention. 
...
By the definition of a clause, it appears that the context $\Delta$ needs to be fully elaborated beforehand but with elaborator at our disposal it does not.
