...
In this section, we will show that how \emph{elaborator reflection} and generic programs are coordinated.
First, metaprograms are used to translate representations (\Cref{sec:parameters}) used by generic programs such as |Tel : Level → Set|, which are \emph{higher-order} and \emph{typed}, and those by the reflection framework which are \emph{first-order} and \emph{uni-typed}.
Then, connections (\Cref{sec:connections}) are generated by walking through two different types of representations to compute the information from each other. 
Finally, generic programs |fold-base F| are \emph{specialised during elaboration} to derive definitions comparable, if not identical, to hand-written ones. 

We should, however, briefly sketch the reflection framework in Agda (\Cref{sec:elab}) first and then explain how this is used in subsequent sections (\Cref{sec:translation,sec:connection-generation,sec:specialising}).


\subsection{Elaborator Reflection in Agda}
\label{sec:elab}

Elaborator reflection is a metaprogramming paradigm introduced by \citet{Christiansen-elaborator-reflection} in which the elaborator machinery can be used by metaprograms directly implemented in Idris, Agda~\cite{Norell2016}, and Lean 4.\todo{citation?}
...
\Viktor{The |TC| monad in Agda, accompanied by primitive operations of types that end in |TC A| (|A| can be any type depending on the operations in question), provides us an interface to utilities which are otherwise only accessible to the elaborator.}

\Viktor{For examples, |checkType : Tm → Type → TC Tm| is a primitive that checks if the second argument is the type of the first one, then returns a type-checked term whose constraints and metavariables are solved if possible.  |getContext : TC (List (Arg Type))| returns the types in the current type-checking context.  |extendContext : ∀ {a} {A : Set a} → Arg Type → TC A → TC A| extends the current context with one type and evaluates the given metaprogram in it.  }

\Viktor{There are multiple ways to run a metaprogram (|TC| computation).
...
Listℕ : Set
Listℕ = evalTC (unquoteTC (def (quote List) [ vArg (def (quote ℕ) []) ]))
\end{code}
\caption{A macro example}
\label{fig:macro}
\end{figure}
\Viktor{Cite https://github.com/UlfNorell/agda-prelude}

\Viktor{Types of macros are required to end in |Tm → TC ⊤| as known as |Tactic|.
...
\Viktor{For example, the macro |evalTC| in \cref{fig:macro} evaluates the metaprogram that is the first argument, gets the reflected representation of the result, and unifies the metavariable at call site with it.
|Listℕ| is defined as the result of the metaprogram |unquoteTC ...| which is judgementally equal to |List ℕ|.}

\Viktor{The primitive |quoteTC : {a : Level} {A : Set a} → A → TC Tm|
returns the reflected representation of the given term of type |A|.
Conversely, |unquoteTC : {a : Level} {A : Set a} → Tm → TC A| takes a reflected representation and translate it to a native term.
|unify : Tm → Tm → TC ⊤| unifies the first term with the second one, and solves metavariables if possible.}

\Viktor{Do we really need |Tm|? For now |Tm| and |Term| are used interchangeably.}

\Viktor{For the convenience of explanation, we present |Tm|, a simplified version of Agda's reflected representation (\cref{fig:reflected term} and \cref{fig:reflected pattern}) instead of the actual implementation (\cref{fig:full reflected syntax}):}
...
\end{enumerate*}
\Viktor{Note that terms represented by |t| are not guaranteed be of type |Level|, because the representation is unityped.}
\Viktor{Checking reflected terms containing |unknown| is equivelant of checking hand-tailored terms containing underscores |_|.}
...
\Viktor{For a macro 
|f : Tm → Name → Bool → Tm → TC ⊤|,
|f u v w| desugars to |unquote (f (quoteTerm u) (quote v) w)|.}
\Viktor{For example, |quoteTerm ℕ| is judgementally equal to |def (quote ℕ) []|, albeit their different appearences to programmers.
It's recommanded to avoid using these top-level primitives directly so we won't explore them further.}

\Viktor{Now we can check that the reflected term in \cref{fig:macro}
|def (quote List) [ vArg (def (quote ℕ) []) ]|
