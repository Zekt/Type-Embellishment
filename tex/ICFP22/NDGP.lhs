...
First, we use macros to translate representations described in \Cref{sec:parameters} used by generic programs such as |Tel : Level → Set|, which are \emph{higher-order} and \emph{typed}, and those by the reflection framework which are \emph{first-order} and \emph{uni-typed} in \Cref{sec:translation}.
Then, connections in \Cref{sec:DataC,sec:FoldC} are generated by walking through two different types of representations at the same time in \Cref{sec:connection-generation}. 
Finally, generic programs such as~\eqref{eq:fold-base-before} are normalised during elaboration to derive definitions that are comparable, if not identical, to hand-written definitions like~\eqref{eq:fold-base-after} in \Cref{sec:specialising}

We should, however, sketch the basic design of the reflection framework in \Cref{sec:elab} first. 

\subsection{Elaborator Reflection in Agda}
\label{sec:elab}
Agda's reflection API includes a set of primitives based on the elaborator monad, called |TC|, which operates on the reflected language formed by |Term| (\Cref{fig:reflected-term}), |Pattern| (\Cref{fig:reflected pattern}), |Literal| (\Cref{fig:other-type}), |Clause| (\Cref{fig:clause}), and |Definition| (\Cref{fig:definition}) corresponding to each syntactic category.
...
Instead we can unquote a reflected expression and bind it to an (object-level) variable.
...
\subsection{Translation between Typed Higher-Order and Uni-typed First-Order Representations}\label{sec:translation}

Our generic programs are given in typed and higher-order representations, so in order to use macros to generate native datatypes and definitions we have to first translate them to its corresponding first-order and uni-typed representation used by reflection.
Take telescopes for example. 
A reflected telescopes is just a list of reflected types while an inhabitant |T : Tel ℓ| (\Cref{fig:telescopes}) used as part of the datatype representation |DataD| may consists of functions.

One obvious approach is to analyse the reflected expressions.
For instance, we would need to translate the quoted expression of |[ A ∶ Set ] [ R ∶ (A → A → Set) ] []|
...
Instead of quoting the entire telescope |T| at once, we choose to pattern match against |T : Tel ℓ| first and compute |e : Term| when necessary.
...
Extending the context (of the call site) with a \emph{local variable} used throughout in another |TC| computation by the primitive |extendContext| allows us to introduce a variable |x| of an arbitrary type. 
To introduce the variable to the scope of |fromTel|, we then need another primitive |unquoteTC| which interprets an reflected expression |e : Term| to its corresponding inhabitant of type |A|, so the |TC| computation |unquoteTC (var 0 []) >>= (λ x → ...) | within the extended context binds the local variable to |x : A|.
That is, introducing a local variable of a type |A| during macro invocation can be achieved by primitives |unquoteTC| and |extendContext| altogether as a pattern observed by \citet{Chen-Mtac-Agda}.
This pattern occurs frequently in our translations, so we define them explicitly as in \Cref{fig:extendContextT,fig:extendCxtTel}.
...
exCxtT B f = do
  ...
  (HL (extendContext)) `B do
    (HL (unquoteTC (var 0 []))) >>= f `B
\end{code}
\caption{Context extension by a type |A : Set ℓ|} 
\label{fig:extendContextT}
\end{minipage}%
\begin{minipage}[h]{0.5\textwidth}
\begin{code} 
exCxtTel : (T : Tel ℓ) → (⟦ T ⟧ᵗ → TC A) → TC A
exCxtTel []        f  = f tt
exCxtTel (A ∷ T)   f  = (HL (exCxtT) A) λ _ x →
  exCxtTel (T x) (λ t → f (x , t))
exCxtTel (T ++ U)  f  = exCxtTel T λ t →
  exCxtTel (U t) (λ u → f (t , u))
\end{code}
\caption{Context extension by a telescope |T : Tel ℓ|} 
\label{fig:extendCxtTel}
\end{minipage}
\end{figure}

For the other direction of the translation, as only a list of reflected types are given there is no loss of information if we just generate the reflected expression |e : Term| of the corresponding telescope |T : Tel ℓ|.
